"""
atlas-ops-broker (PROD) — extended with autonomy commands.
...
def cmd_ssm_read(args):
    """Read one or more SSM parameters from /atlas/*."""
    ...
    for n in names:
        if not n.startswith("/atlas/"):
            ...
            continue
        try:
            r = ssm.get_parameter(Name=n, WithDecryption=with_decryption)
...
def cmd_rds_describe(args):
    """List RDS clusters + instances. Optional filter by identifier prefix."""
...
def cmd_rds_restore(args):
    ...
    # Refuse if the new cluster already exists
    try:
        rds.describe_db_clusters(DBClusterIdentifier=new_cluster_id)
    ...
    r = rds.restore_db_cluster_from_snapshot(
